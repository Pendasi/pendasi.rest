...
class MigrationRunner {
    ...
    private function runMigration(string $file, string $method): void {
        ...
        $className = $this->resolveMigrationClassFromFile($file);
    ...
    private function resolveMigrationClassFromFile(string $file): string {
        ...
        // Cas rollback: les classes peuvent déjà être chargées (require_once ne re-déclare rien).
        // On utilise alors Reflection pour retrouver la classe dont le fichier source correspond.
        $targetFile = realpath($file) ?: $file;
        foreach ($after as $className) {
            ...
            try {
                ...
                if ((realpath($classFile) ?: $classFile) === $targetFile) {
                    return $className;
                }
        ...
        // Fallback: ancienne convention (peut échouer mais on garde la compat)
        $fallback = $this->getClassNameFromFile($file);
